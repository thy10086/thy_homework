...
// ---------- 加解密函数 ----------
void SM4_encrypt(const uint32_t input[4], const uint32_t rk[32], uint32_t output[4]) {
    uint32_t X[36];
    memcpy(X, input, 16);
    for (int i = 0; i < 32; ++i)
...
void SM4_decrypt(const uint32_t input[4], const uint32_t rk[32], uint32_t output[4]) {
    uint32_t X[36];
    memcpy(X, input, 16);
    for (int i = 0; i < 32; ++i)
...
int main() {
    ...
    cout << (correct ? "✅ Decryption successful!" : "❌ Decryption failed!") << endl;
